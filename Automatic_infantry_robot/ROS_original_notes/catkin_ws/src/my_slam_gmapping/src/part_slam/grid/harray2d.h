...
//调整存储 “地图补丁” 的二维数组的大小，并且对存储的指向Array2D<Cell>的指针初始化指向
//初始化
template <class Cell>
void HierarchicalArray2D<Cell>::resize(int xmin, int ymin, int xmax, int ymax)
{
	...
	for (int x=0; x<xsize; x++)
	{
		...
		{
			newcells[x][y] = std::shared_ptr< Array2D<Cell> >(0);//对地图中每一个地图补丁patch指向为空
		}
	}
	...
	for (int x=dx; x<Dx; x++)
	{
		for (int y=dy; y<Dy; y++)
		{
			newcells[x-xmin][y-ymin]=this->m_cells[x][y];//对已经分配过内存的地址转移指向
		}
...
Cell& HierarchicalArray2D<Cell>::cell(int x, int y)
{
	...
	std::shared_ptr< Array2D<Cell> >& ptr=this->m_cells[c.x][c.y];//patch的引用
